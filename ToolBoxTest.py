...
from DataSetTest import DataSet
import pydicom
import numpy as np
...
class ToolBox(DataSet):
    ...
    def convert_to_nrrd(self, export_path: str, region_of_interest: str = 'all', image_type = np.int16):
        ...
        if self._data_type == 'dcm':
            ...
            for pat,pat_path in tqdm(self,desc='Patients converted'):
                ...
                if self._image_only == True:
                    img_path = pat_path[0]
                    
                    for x in img_path:
                        try:
                            image = self.__get_image_no_mask(img_path,image_type)
                            print(image)
                            print("between the array/image variables if ti worked")
                            export_dir = os.path.join(export_path,'converted_nrrds',pat)
                            if not os.path.exists(export_dir):
                                os.makedirs(export_dir)
                            image_file_name='image.nrrd'
                            ...
                            warn('Patients %s image : %s skipped'%(pat))
    ...
    def __get_image_no_mask(self,img_path,image_type):
        image,_ = self.__read_scan(img_path)
